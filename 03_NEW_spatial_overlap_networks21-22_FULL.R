...
for(i in 1:length(overlap_network_list)) {
  ...
  for(j in 1:length(overlap_network_list[[i]])){
    ...
    data <- overlap_network_list[[2]][[3]]

    site.id <- names(overlap_network_list[2])
    month.id <- names(overlap_network_list[[2]][3])
   ...
   ## Matt M-S suggestion - color edges by weight, thickness by weight
    ...
    # g <- set.vertex.attribute(graph=g, name="sb", index=V(g), value=netmeta$sb)
      ...
      theme_void() +
      labs(title=paste(names(overlap_network_list[[2]])[3]),
...
for(i in 1:length(overlap_network_list)) {
  ...
  png(filename = paste("spatial_overlap_", "STSB_wt0.05byTHICK3_", names(overlap_network_list)[[i]], "_2021", ".png", sep = ""),
      width=10 , height=3, units="in", res=600)
