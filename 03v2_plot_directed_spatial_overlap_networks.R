## NEW FILE!! This is for the directed network on HR circles
...
for(i in 1:length(overlap_network_list)) {
  ...
  for(j in 1:length(overlap_network_list[[i]])){
    ...
    tidyg <- as_tbl_graph(data, directed=TRUE) %>% left_join(netmeta, by=c('name'='tag'))
    ...
    monthgraph <- ggraph(tg, layout=centroidmat) + #set 'layout' to matrix of monthly centroids
      ...
      # geom_edge_fan(arrow=arrow(length = unit(4, 'mm')), end_cap = circle(3, 'mm')) + #curved edges, option to add arrows
