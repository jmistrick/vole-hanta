## Updated for final publication (Jan 2024) - using the 05.04.23 version of trapping/WR data 
## this code was run to generate the 2021 "fulltrap" file that only has the necessary columns for running
## the code needed in the JAE "Vole Spatial..." manuscript
## THIS FILE IS NOT PART OF THE GIT REPO that was archived on Zenodo (because it uses the full data)
## JEM ran this code to generate the finalfinal version of fulltrap21 and then used that csv file for the 
## code published with the manuscript

## This code was also updated from what is in the vole-spatial-v2 folder because it only needs to run 2021 data
## for the JAE manuscript (so any code for 2022 data was removed)
...
yr=2021
fulltrap_output = "fulltrap21_volecapturedata.rds"
#########################################################
...
##          year = number (2021) to get the year you want
...
####################################################################################################
  ...
  #save fulltrap to rdata file so I can pull it for other scripts
  saveRDS(fulltrap, file = here(fulltrap_output))
  
  #save data to csv for uploading to DRYAD
  write.csv(fulltrap, file="fulltrap21_volecapturedata.csv")
